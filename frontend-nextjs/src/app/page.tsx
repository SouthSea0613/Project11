...
const FEATURED_SLUGS = ["planit", "aws-infra-modernization", "rd-autonote"];
    ...
    <main className="relative">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,240px)_minmax(0,1fr)_minmax(0,340px)] lg:items-start">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-7 flex flex-wrap items-center gap-2">
              ...
              <Link
                href="/projects"
                className="rounded-md border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:border-emerald-400/50 hover:bg-white/10"
              >
                프로젝트 {publicProjects.length}개
              </Link>
              <a
                href={`mailto:${namhaeContact.email}`}
                className="rounded-md border border-emerald-400/30 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-200 transition hover:bg-emerald-500/20"
              >
                이메일
              </a>
            </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2 text-xs">
              ...
              <a
                ...
                className="inline-flex items-center gap-1 rounded-md border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 font-medium text-emerald-300 transition hover:border-emerald-400/60"
              >
                PlanIT
      ...
      <section className="mx-auto max-w-screen-2xl px-4 pb-20 sm:px-6 md:px-8 lg:px-6">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {namhaeSkills.slice(0, 14).map((skill) => (
            <span
              key={skill}
              className="rounded-full border border-emerald-400/25 bg-emerald-500/10 px-2.5 py-0.5 text-[11px] font-medium text-emerald-200"
            >
              {skill}
            </span>
          ))}
      ...
      <section className="mx-auto max-w-screen-2xl px-4 pb-12 sm:px-6 md:px-8 lg:px-6">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Link
            href="/Namhae_Kim"
            className="group flex flex-col justify-between rounded-2xl border border-emerald-400/30 bg-slate-950/60 p-5 transition hover:-translate-y-0.5 hover:border-emerald-400/70"
          >
            <div>
              <p className="text-[10px] font-semibold tracking-widest uppercase text-emerald-400">
                Profile
            ...
            <span className="mt-3 text-xs text-emerald-400">열기 →</span>
          </Link>
        </div>
      </section>
        ...
        <div className="grid gap-6 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-6 md:p-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] lg:gap-10">
          <div>
            ...
            <p className="mt-3 text-sm text-slate-200 md:text-base">
              백엔드·풀스택 개발, AI·RAG 파이프라인, B2B SaaS 아키텍처 관련
